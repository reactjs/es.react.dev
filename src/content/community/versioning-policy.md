...
Esta política de versiones describe nuestro enfoque para los números de versión de paquetes como `react` y `react-dom`. Para una lista de versiones anteriores, consulta la página de [Versiones](/versions).
...
Sabemos que nuestros usuarios continúan usando versiones antiguas de React en producción. Si descubrimos una vulnerabilidad de seguridad en React, lanzamos una corrección retroportada para todas las versiones principales que se ven afectadas por la vulnerabilidad.
