...
No hay nada "malo" acerca de usar estado o métodos del ciclo de vida en componentes. Como cualquier funcionalidad poderosa, deben ser usados en moderación, pero no tenemos intención de removerlos. Por el contrario, pensamos que son parte integral de lo que hace útil a React. Podríamos habilitar [patrones más funcionales](https://github.com/reactjs/react-future/tree/master/07%20-%20Returning%20State) en el futuro, pero tanto el estado local como los métodos del ciclo de vida serán parte de ese modelo.

Los componentes son a menudo descritos como "solo funciones", pero desde nuestro punto de vista necesitan ser más que eso para ser útiles. En React, los componentes describer cualquier comportamiento componible, y esto incluye el renderizado, el ciclo de vida y el estado. Algunas bibliotecas externas como [Relay](http://facebook.github.io/relay/) aumentan los componentes con otras responsabilidades tales como describir las dependencias de datos. Es posible que esas ideas también sean integradas en React de alguna manera.
...
Es por eso que algunas veces añadimos funcionalidades a React. Si notamos que muchos componentes implementan una funcionalidad en específico de manera ineficiente o incompatible, podríamos preferir incluirla en React. No lo hacemos a la ligera. Cuando lo hacemos, es porque estamos confiados que elevar el nivel de la abstracción beneficia el ecosistema en su conjunto. El estado, los métodos del ciclo de vida, y la normalización de eventos entre navegadores son ejemplos claros de esto.
...
Si queremos descontinuar un patrón que no nos gusta, es nuestra responsabilidad considerar todos los casos de uso existentes para el, y [educar a la comunidad respecto a las alternativas](/blog/2016/07/13/mixins-considered-harmful.html) antes de descontinuarlo. Si algún patrón es útil para construir aplicaciones, pero es difícil de expresar de una manera declarativa, [proveeremos un API imperativo](/docs/more-about-refs.html) para el. Si no podemos encontrar una API perfecta para algo que consideramos necesario en múltiples aplicaciones, [proporcionaremos una API funcional temporalmente](/docs/legacy-context.html) siempre y cuando sea posible librarnos de ella posteriormente, y deje la puerta abierta a mejoras futuras.
...
Dado el inmenso tamaño de la base de código de Facebook en React, una migración interna exitosa es a menudo un buen indicador de que otras compañías tampoco tendrán problemas. Sin embargo, a veces las personas nos indican casos de usos adicionales que no habíamos pensado, y añadimos válvulas de escape para ellos o repensamos nuestro enfoque.

No descontinuamos nada sin ninguna buena razón. Reconocemos que a veces las advertecias de descontinuación causan frustración, pero las añadimos porque las descontinuaciones limpian el camino para las mejoras y nuevas funcionalidades que nosotros y muchos en la comunidad consideran valiosas.

Por ejemplo, en React 15.2.0 añadimos [una advertencia acerca de props desconocidas en el DOM](/warnings/unknown-prop.html). Muchos proyectos fueron afectados por esto. Sin embargo arreglar esta advertencia es importante para poder introducir soporte para [atributos personalizados](https://github.com/facebook/react/issues/140) en React. Hay una razón como esta detrás de cada advertencia de descontinuación que añadimos.
